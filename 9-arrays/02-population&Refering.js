...
console.log(`hello ${name}, youo look nice today.`);

//recall for-of loop -> gives the values if the array
...
console.log(food);
food.splice(2, 0, 'Sweet Potato Pie'); //asks for an insertion point, how many things to remove, them the item or items to add
console.log(food);
...
//forEach allows us to iterate with loops specifically, and we can directly grab both the elemments
//and thier numbers
...
//let's so the following with an array:
//we will check if we are working with an array
//flip the values within the array (what was in index 4 is now in 0, 3 to 1, ect.)
